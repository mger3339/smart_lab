...
<div id="<?php echo $row_id; ?>" class="row">
	...
	<div class="section row-btns">
		...
		<?php echo form_open("super-admin/client-applications/{$row->client_id}/delete/{$row->id}", array('class' => 'delete-row-form', 'data-confirm' => "Are you sure you would like to remove this client application? All data relative to this client application will be deleted permanently.\n", 'data-re-confirm' => "Are you absolutely sure you would like to remove this client application?\n")); ?>
			<button class="delete-row-btn" type="submit" name="delete" value="TRUE">Delete</button>
		<?php echo form_close(); ?>
		<button class="edit-row-btn" data-url="<?php echo "super-admin/client-applications/{$row->client_id}/edit/{$row->id}"; ?>">Edit</button>
		<div class="clear"></div>
	</div>
	...
	<div class="section toggle-section">
		...
		<h4><span class="toggle-btn"><?php echo $row->original_name; ?> modules </span><a href="<?php echo site_url("super-admin/client-application-modules/{$row->client_id}/{$row->id}"); ?>">[edit]</a></h4>
